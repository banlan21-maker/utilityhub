...
export async function generateMetadata({
  ...
  const title = isKo
    ? "단리·복리 이자 계산기 | Utility Hub"
    : "Simple & Compound Interest Calculator | Utility Hub";
  const description = isKo
    ? "단리와 복리 이자를 즉시 계산하고 연도별 수익 그래프로 복리 효과를 시각화하세요."
    : "Calculate simple and compound interest instantly with annual growth charts and APY comparison.";
  const canonical = `https://www.theutilhub.com/${params.locale}/utilities/finance/interest-calc`;
...
const faqSchema = {
  ...
  "mainEntity": [
    ...
    { "@type": "Question", "name": "실효 이자율(APY)이란 무엇인가요?", "acceptedAnswer": { "@type": "Answer", "text": "복리 효과를 반영한 실제 연간 수익률입니다. 명목 이자율이 5%라도 월복리로 계산하면 실효 이자율은 약 5.12%가 되어, 실제로는 더 높은 수익을 얻게 됩니다." } },
    { "@type": "Question", "name": "대출 이자도 이 계산기로 계산할 수 있나요?", "acceptedAnswer": { "@type": "Answer", "text": "네, 가능합니다. 단, 대부분의 대출은 원리금 균등상환 방식으로 매월 원금과 이자를 함께 갚기 때문에 본 계산기의 단순 이자 계산과는 차이가 있을 수 있습니다. 정확한 대출 상환 계획은 은행의 대출 상담을 권장합니다." } }
  ]
};
