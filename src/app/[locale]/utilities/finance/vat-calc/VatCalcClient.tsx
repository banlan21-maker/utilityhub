...
export default function VatCalcClient() {
  ...
  const handleCopy = () => {
    const lines = mode === 'add'
      ? [
          `${isKo ? '공급가액' : 'Supply'}: ${fmt(val)}${isKo ? '원' : ''}`,
          `VAT (${rate}%): ${fmt(vatAmount)}${isKo ? '원' : ''}`,
          `${isKo ? '합계' : 'Total'}: ${fmt(totalAmount)}${isKo ? '원' : ''}`,
        ]
      : [
          `${isKo ? '합계' : 'Total'}: ${fmt(val)}${isKo ? '원' : ''}`,
          `${isKo ? '공급가액' : 'Supply'}: ${fmt(supplyAmount)}${isKo ? '원' : ''}`,
          `VAT (${rate}%): ${fmt(extractedVat)}${isKo ? '원' : ''}`,
        ];
  ...
  return (
    <div className={s.vat_container}>
      ...
      <section className={s.vat_panel}>
        ...
        {val > 0 && (
          <div className={s.vat_result_card} style={{ animation: 'bounceIn 0.5s ease-out' }}>
            ...
            <div className={s.vat_copy_row}>
              <button
                className={`${s.vat_copy_btn} ${copied ? s.vat_copy_btn_done : ''}`}
                onClick={handleCopy}
      ...
      <div style={{ width: '100%' }}>
        ...
        <RelatedTools toolId="finance/vat-calc" />
        <div className={s.vat_ad_placeholder}>AD</div>
        <SeoSection
          ko={{
